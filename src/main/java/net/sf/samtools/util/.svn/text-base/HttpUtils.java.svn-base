...
public class HttpUtils {
    ...
    public static void printHeaderFields(final URL url) {
        ...
        try {
            ...
            for (final String name : conn.getHeaderFields().keySet()) {
                LOGGER.info(name + "\t" + conn.getHeaderField(name));

            }
        } catch (Exception e) {
            LOGGER.error("printHeaderFields",e);
        }
        finally {
            if (conn != null && conn instanceof HttpURLConnection) {
                ((HttpURLConnection) conn).disconnect();
    ...
    public static boolean resourceAvailable(final URL url) {
        ...
        try {
            ...
            LOGGER.error("resourceAvailable",e);
            return false;
        }
        finally {
            if (conn != null && conn instanceof HttpURLConnection) {
