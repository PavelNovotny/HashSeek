 ...
 */
public class TimeFileEvaluator implements FileEvaluator {
...
//        Pattern pattern = Pattern.compile("^(other|jms)_s._alsb_aspect.time\\.(\\d{8})(\\.\\d{2})?(.bgz)?$");
        Pattern pattern = Pattern.compile("^(other|jms)_s._alsb_aspect.time\\.(\\d{8}|part)(\\.\\d{2})?(.bgz)?$");
        Matcher matcher = pattern.matcher(name);
        if (appArguments.getDatesToScan().isEmpty()) {
            return false;
        } else if (matcher.matches()) {
            return (appArguments.getDatesToScan().contains(matcher.group(2)) || "part".equals(matcher.group(2)));
        } else {
            return false;
        }
    }
